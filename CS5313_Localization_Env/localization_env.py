...
class Environment:
    ...
    heading_transitions     -- The table of transition probabilities for the headings given each cell. Format is [x][y][heading][probability] which will return the probabilities of each heading for the next time step given the robot's current x, y, and heading.\n
    robot_location          -- The current location of the robot, given as a tuple in the form (x, y).
    ...
    def update(self, location_probs: list[list[int]], headings_probs: dict[Headings, float]):
        ...
        Args:
            location_probs (list[list[int]]): The probability of the robot's current heading being any given heading. Created from your project code. Format dict{<Headings enum> : float, <Headings enum> : float,... }
            headings_probs (dict[Headings, float]): The probability of the robot's current heading being any given heading. Created from your project code. Format dict{<Headings enum> : float, <Headings enum> : float,... }
        """
        if self.running:
            self.game.update(
                self.map,
